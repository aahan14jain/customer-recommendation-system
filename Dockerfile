...
CMD ["sh", "-c", "python manage.py migrate && python manage.py collectstatic --noinput && python manage.py load_data --file predictor/data/dataset1.csv && python manage.py sync_customer_accounts --password 1234 && exec gunicorn customer_prediction_system.wsgi:application --bind 0.0.0.0:$PORT"]
